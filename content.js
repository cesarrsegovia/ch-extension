...
function processTweet(tweet) {
  ...
  if (userNames && userNames.innerText.includes(TARGET_ACCOUNT)) {
    ...
    if (
      tweetText &&
      tweetText.innerText.toLowerCase().includes("premier league")
    ) {
      type = "premier_league";
    } else if (
      tweetText &&
      tweetText.innerText.toLowerCase().includes("sportsbook")
    ) {
      type = "sportsbook";
    } else if (
      tweetText &&
      (tweetText.innerText.toLowerCase().includes("gaming") ||
        tweetText.innerText.toLowerCase().includes("game"))
    ) {
      type = "games";
    }

    console.log("Gamblor Detector: Tweet detectado de Gamblor Casino");
    console.log({ text: tweetText.innerText.toLowerCase(), type: type });
...
function highlightTweet(tweet, type = "default") {
  ...
  if (!buttonText) {
    return; // no crear botón si no hay texto definido
  }
  ...
  if (actionsBar) {
    ...
    const tooltipHtml =
      type === "premier_league" || type === "sportsbook"
        ? `
        ...
        : `
         <div class="gamblor-tooltip">
             <span style="color: #f2b90d;">New games every week!</span>
            </div>`;

    btn.innerHTML = `${imgIcon}<span class="gamblor-btn-text">${buttonText}</span>${tooltipHtml}`;
